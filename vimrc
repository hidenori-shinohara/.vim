...
"
" See the URL above for the detailed list of macros provided or just browse
...
let g:macros = '\<\(tab\|bar\|ben\|bit\|beq\|bqn\|bex\|bfi\|bfg\|bfe\|btb\|bta\|pic\|mat\|frac\|dot\|ddot\|vb\|verb\|bf\|em\|it\|mb\|sq\|todo\|bpr\|mbeq\|mbal\|bal\)\>'
...
"function! s:Dollar() let s:insert = "$" let s:left = getline(line("."))[col(".")-2] if s:left == '$' let s:insert = "$\<Left>" endif return s:insert endfunction inoremap <buffer> $ <C-R>=<SID>Dollar()<CR> 
" typing == results in &=& iab <buffer> == &=&
